...
def get_bb(img,mask):
    ...
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)  # cv2.RETR_EXTERNAL 定义只检测外围轮廓
    delete_list=[]
    if len(contours) == 0:
    ...
    # cv2.imshow('a', img)
    # cv2.waitKey()
    # cv2.imshow('b', img_new)
    # cv2.waitKey()

if __name__ == '__main__':
    img_dirs = sort_humanly(os.listdir('output'))
    mask_dirs = sort_humanly(os.listdir('labeled\output'))
    for img_dir ,mask_dir in zip(img_dirs,mask_dirs):
        


        imgs =sort_humanly(glob.glob(f'output/{img_dir}/*.jpg'))
        masks =sort_humanly(glob.glob(f'labeled\output\{mask_dir}/*.jpg'))
